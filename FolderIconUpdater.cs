...
internal static class FolderIconUpdater
{
	...
	internal static void SetFolderIcon(string folderPath, string iconPath)
	{
		...
		{
			throw new ArgumentException("Invalid folder path or icon path.");
		}
		...
		try
		{
			...
			iconPathPtr = Marshal.StringToHGlobalAuto(iconPath);
		...
		finally
		{
			...
			if (iconPathPtr != IntPtr.Zero)
			{
				Marshal.FreeHGlobal(iconPathPtr);
				Marshal.FreeHGlobal(fcsPtr);
			}
		}
	}
}